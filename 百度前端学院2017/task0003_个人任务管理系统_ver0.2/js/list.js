...
/**
 * 新建分类，在当前选中的目录下新建分类
 * @param {[string]} className [新添加类的类名]
 */
List.addNewTypeList = function(className){

	var add_class_name = className;
	...
	if(now_click.className.indexOf('allClass')>=0){
		...
		if(add_class_name != null){
			...
			var new_class_list_item = List.ClassListItem(add_class_name,'1stClass');
			add_to_location.appendChild(new_class_list_item);

			var new_class_list = document.createElement('ul');
...
*/
			var new_class_list_item = List.ClassListItem(add_class_name,'2ndClass');
			add_to_location.appendChild(new_class_list_item);
...
/**
 * 右侧编辑区域的显示
 * @param  {Boolean} isShow [是否显示编辑区域]
 * @param  {Boolean} isNew  [触发按钮是否为 新建任务]
 * @return {[type]}         [显示或隐藏编辑区域]
 */
List.editAreaShow = function(isShow,isNew){
	isShow = isShow || '';
	isNew = isNew || '';
	...
	if(isShow == true){
		...
		utility.qs(".todo_title_display").style = "display:none";
		utility.qs(".todo_date_display").style = "display:none";
		utility.qs(".todo_content_div").style = "display:none";
	}else{
		//隐藏编辑区域
...
/**
 * 新建左侧栏分类列表表项
 * @param {[String]} className [新建类的类名]
 * @param {[String]} level     [新建类的等级（一级类或二级类）]
 * @return {[Node]} [返回包含该类名的li节点]
 */
List.ClassListItem = function(className,level){
	var new_class_list_item = document.createElement('li');
	var temp_class_list_item = '<i class=""{{iconClassName}}""></i>'
			   				 + '<span class="_"{{className}}" "{{classLevel}}"">"{{className}}"</span>'
			   				 + '<i class="delete_icon"></i>';

	var replace_reg = new RegExp('"{{className}}"', 'g');//replace()第一次参数传入字符串时，默认只替换第一个，若传入是正则，可用/g
	temp_class_list_item = temp_class_list_item.replace(replace_reg, className);
	temp_class_list_item = temp_class_list_item.replace('"{{classLevel}}"', level);
	if(level == "1stClass"){
		//第一类时的图标
		temp_class_list_item = temp_class_list_item.replace('"{{iconClassName}}"','type_file_icon');
	}else{
		//第二类时的图标
		temp_class_list_item = temp_class_list_item.replace('"{{iconClassName}}"','type_item_icon');
	}
	new_class_list_item.innerHTML = temp_class_list_item;
	return new_class_list_item;
}
	...
	for(var i=0,len=class_1.length;i<len;i++){
		if(class_1[i] != null){
			if(class_1[i] != '默认分类'){
				...
				//新建一个类列表表项
				var new_class_list_item = List.ClassListItem(this_class_name_1, "1stClass");
			...
			if(class_2[i] != null){
				...
				for(var j=0,len2=class_2[i].length;j<len2;j++){
					...
					//要新添加表项位置的父元素
					//console.log(class_1[i],utility.qs('._' + class_1[i]));
					add_to_location_2 = utility.$parent(utility.qs('._' + class_1[i]),'li').nextSibling;
									
					//新建一个类列表表项
					var new_class_list_item = List.ClassListItem(this_class_name_2, "2ndClass");

					add_to_location_2.appendChild(new_class_list_item);
				}
			}
		}
	}
...
/**
 * 左侧栏目标类名的选中
 * @param  {[string]} className [指定左侧栏的目标类名，以便自动选中该类]
 * 
 */
List.leftClass = function(className){	
	...
	//中间栏任务标题的显示
	var info = List.queryClassInfo();
	List.listDisplay(info.className , info.classLevel);
}
...
List.deleteClass = function(){
		if(confirm("删除操作不可逆，是否要删除该项？")){
			...
			//获得要删除类的类级，一级or二级类
			var delete_class_level = List.queryClassInfo(utility.qs('span',delete_class)).classLevel;
			
			//删除类标签
			if(delete_class_level == true){
				//删除类为第一类
...
List.deleteListItem = function(){
	...
	targetId = targetId.split("").slice(1).join('');//去掉_
	//console.log(targetId);
...
/**
 * 发生改变时，中间栏任务标题的显示
 * @param  {[String]} thisClass [指定某一类名，显示其下的任务标题]
 * @param  {[String]} boolean   [某一类名是否为一级分类，true为一级分类，false为二级分类]
 * @param  {[String]} view      [['','true','false'] 指定 所有、未完成、已完成 中的一项，显示其下的任务标题]
 * 
 */
...
/*	    var now_click = utility.qs('.list_item_select');
	    ...
	    var now_click = utility.qs('.list_item_select');
	   	var info = List.queryClassInfo();
	    if(info.classLevel == true){
	    	//只有一级分类
	    	updateData.class_1 = info.className;
	    } else if(info.className == '默认分类'){
	    	//默认分类
	    	updateData.class_1 = '默认分类';
	    } else{
	    	//有两级分类
	    	updateData.class_2 = info.className;
	    	var previousBro = utility.$parent(now_click,'ul').previousSibling;
	    	updateData.class_1 = utility.qs('span',previousBro).innerHTML;
	    }
		...
		var this_newtodo_id = utility.qs('.todo_title_span').id;
		console.log(this_mid_list_li,this_newtodo_id);
...
List.completeTodo = function(){
	...
	//中间栏的显示更新
	var class_info = List.queryClassInfo();
	console.log(class_info);
	List.listDisplay(class_info.className, class_info.classLevel, class_info.classView);
...
/**
 * 查询 当前左侧栏给定分类的类名 和 类等级 和 中间栏当前选中的view（所有、未完成、已完成）
 * @return {[object]} [返回info{className，classLevel, classView}]
 */
List.queryClassInfo = function(thisClass){

	thisClass = thisClass || utility.qs('.list_item_select')
	//console.log(thisClass);

	var info = {};
	var now_select_class = thisClass;
	var now_select_class_name = now_select_class.innerHTML;
	var now_mid_select = utility.qs('.mid_header_select').innerHTML;

	//获得当前选中的类名
	if(now_select_class_name == '所有任务' || now_select_class_name == '分类列表'){
		//当前左侧栏选择为所有任务或任务列表
		...
		info.className = now_select_class_name;
		//获取当前选中的类级
		if(now_select_class.className.indexOf('1stClass')>=0){
			//一级分类
			info.classLevel = true;
		}else{
			//二级分类
			info.classLevel = false;	
		}
	}

	//获取中间栏的view
	if(now_mid_select == '未完成'){
		//未完成
		info.classView = 'false';
	}else if(now_mid_select == '已完成'){
		//已完成
		info.classView = 'true';
	}else{
		//所有
...
List.listAddClickEvent = function(){
	...
	//给左侧 新建分类图标 绑定事件
	utility.$on(utility.qs('.add_list'),'click',function(){
		var add_class_name = prompt('请输入新建分类的名称');
		List.addNewTypeList(add_class_name);
	});
	
	//给左侧 新建任务图标 绑定事件
	utility.$on(utility.qs('.add_item'),'click',function(){
		List.editAreaShow.call(window,true,true);
	});
